...
{
    NODE *curr = start;
    while (curr != NULL) // traversing forward till second last node
    {
        printf("%d\t", curr->value);
        curr = curr->next;
    }
    // while (curr != NULL) // traversing backward from last to first node
    // {
    //     printf("%d\t", curr->value);
    //     curr = curr->prev;
    // }
...
void Reverse_list()
{
    NODE *current, *ptr = NULL, *tail;
    current = start;

    while (current != NULL)
    {
        // exchange the address of previous and next pointer of each node to reverse the dir of list
        current->prev = current->next;
        current->next = ptr;

        ptr = current;           // ptr points to the previous node of the current node
        current = current->prev; // move to the next node since prev and next were swapped
    }
    start = ptr;
}
